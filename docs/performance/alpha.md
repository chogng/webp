...
| A18 | licensed real-image alpha corpus + filter-policy/owner census：建立来源、许可、native/derived 与重复簇边界，验证是否存在下一代架构 | `codex/alpha-real-image-corpus@def7a29e`；measured `73945b46` | intake `401e48080e480600a9b8e94c8a2d8401e94b3a88`；最终有效 main / merge-base `62d8afea6c23a93aec9393b4d746b6dd69d76305` | [`b11f` worktree](</Users/lance/.codex/worktrees/b11f/webp>)；task `019f8a5a-cce8-7141-88c5-fc4014891dcb` | 120 张 / 31,869,241 pixels；screen 1,080/1,080 双 decoder exact；formal 5,400 measurements / 45 RSS processes；native gap 29.3448%，但 conformance 0.0770%、leave-largest 0.9069%、soft-shadow-gradient dominance 84.1813%；报告 `def7a29e:reports/alpha-real-image-corpus/README.md` | **完成 / no A19 direction**；production policy/default 不变，实验 instrumentation 不合并 | 723 项 SHA 通过；保留全部 invalidated runs；顶部基准表不变，回填完整 corpus/license/timing/resource/gate provenance |
...
### A16 exact-sized cursor token sink 最终结果

A16 从本地 `main@12c6fbc9e0d15cc86a5dda8b5466e7c700563110` 精确创建，登记基线为
`92216c0272898b4e71d131ca0f984dd778a817d2`。实验期间 main 经 container、VP8L ledger、
demux、mux/editor 与 decoder-only 多次前进；每次旧 binary、census、screen 和 gates 都
完整移入 `raw/invalidated/`。最终代码在
`main@65fc54e1d411cb673e24066dbb36c87765b1f0db` 上重放，唯一冲突只涉及
`alpha/mod.rs` 的 decoder-only feature gate；main gate 与 A16 encode-only 私有模块均
保留。最终 measured code / evidence / audit / HEAD 为 `d3794de4` / `d7d64166` /
`4897f4f1` / `84ddaaa0`，merge-base 精确等于 final main，分支和 worktree 干净。

候选不增加第二次 token walk：frequency pass 只在 exact/census 变体累计 checked copy
extra bits，最终 Huffman tables 建好后计算 `sum(frequency × actual width) + extras`。
Exact sink 把 sizing、精确分配、zero-fill、prefix copy、cursor setup 和 finish proof 都放在
timed encode 内。四个 same-binary controls 是 current packed、exact-u128、exact-u64 和
exact-u64 + four-word staging 的 combined；没有 file-size/corpus activation threshold。

五轮 owner census 覆盖每轮 71 个 entropy streams、4,356,631 tokens。Current packed
denominator 为 74.793 ms；完整 current sink boundary 只有 7.627 ms（**10.197%**），刚比
Phase-A permission gate 高 0.197 个百分点。现实候选已经反优化：exact-u128 boundary
8.120 ms（+6.466%），exact-u64 16.789 ms（+120.145%），combined 15.632 ms
（+104.970%）。这证明 A15 的 17.528% sink-append 诊断不能转借为 A16 可回收时间。

| A16 variant | ALPH-only median | Change | ALPH p50 / p95 / worst | Regressions | Whole change |
|---|---:|---:|---:|---:|---:|
| current packed | 741.948 ms | reference | reference | reference | reference |
| exact-u128 | 746.588 ms | **+0.625%** | +4.254 / +13.287 / +14.851% | 34/41 | +0.794% diagnostic |
| exact-u64 | 829.817 ms | **+11.843%** | +5.432 / +17.708 / +20.408% | 36/41 | +2.277% diagnostic |
| combined | 815.057 ms | **+9.854%** | +5.263 / +12.397 / +16.327% | **41/41** | +0.979% diagnostic |

12 个有效 fresh-process slots 均输出每 suite 4,118,622 ALPH bytes（5.161501 bpp）和
6,618,910 complete WebP bytes（8.294888 bpp）；ALPH/WebP bytes、checksum、hash 零
mismatch，所有有效 stderr 为空。原 12-slot batch 被会话截断后仍在后台运行，第一次
retry 与其重叠；两套均完整作废，最终三轮只使用互不重叠数据。

唯一正信号是 allocation 诊断：71 streams 的 current reserved capacity
14,194,137 B 降到 exact output 4,200,623 B（-70.406%），单 stream 最大
7,864,399 -> 3,979,859 B（-49.394%）。它不是 peak RSS，不能形成 memory Pareto；正式
CPU/RSS 因 screen reject 未运行。Workspace tests、feature tests（283 passed / 4 ignored）、
focused 39/39、default/feature Clippy `-D warnings`、fmt 和 release build 通过。
...
### A18 licensed real-image corpus 最终结论

A18 独立任务 `019f8a5a-cce8-7141-88c5-fc4014891dcb`、worktree
`/Users/lance/.codex/worktrees/b11f/webp`、分支 `codex/alpha-real-image-corpus` 已完成且干净。
创建/intake base 为 `401e48080e480600a9b8e94c8a2d8401e94b3a88`；主线前进后，最终有效
local main / base / merge-base 为 `62d8afea6c23a93aec9393b4d746b6dd69d76305`，measured commit
为 `73945b46a4324f201679b610bea16008ea7d831b`，最终 evidence/report commit 为
`def7a29e5651a0e9dcb6c87a4537251db62df3b7`，且有效 main 是 final 的祖先。Corpus/oracle pins
分别为 `06ddd96e276c2c638a72d39d3c0f340afd61978c` 与
`733c91e461c18cf1127c9ed0a80dccbcfed599d3`。

冻结 corpus v1 共 **120 张 / 31,869,241 pixels**，五个 strata 各 24 张：84 张 native raster
占 22,712,633 pixels，28 张 derived vector 占 1,144,832，8 张 derived procedural 占
8,011,776；21 张 binary-alpha、99 张 translucent。Original、normalized、RGBA 与 alpha exact
hash 全部唯一，冻结的 30 对近重复复审也全部判定为不同图。许可计数为 CC0-1.0 53、
CC-BY-SA-4.0 20、CC-BY-4.0 19、MIT 16、Apache-2.0 12；每项都有固定 source revision、
source/normalized SHA-256、SPDX、attribution、归一化 recipe 与 license evidence。Kenney 作为一个
聚合 origin 共 40 张，占 all/native pixels 13.206766% / 18.531079%，其固定 pack 明细为
1-Bit Platformer 15、Cursor Pixel 5、Particle 16、UI 4；完整 origin/pack/family census 在
`reports/alpha-real-image-corpus/raw/formal-final/source-census.tsv`。CLIC 因预计 7.48 GiB 下载和
14.96 GiB workspace 明确未纳入。

Screen 共 1,080 个 encoder-policy-image rows，project decoder 与 pinned `dwebp` 均
**1,080/1,080 exact-alpha**，9 个过程均为零 stderr。Formal 使用 5 rotations × 10 encodes，
产生 **5,400 measurements** 与 **45 个独立 `/usr/bin/time -lp` process/RSS records**，零异常退出。
Rust ALPH bytes 为 Fast 3,968,110、None 4,100,607、Horizontal 4,482,572、Vertical 4,243,998、
Gradient 4,427,400、Best 3,872,154；对应 pinned libwebp filtering=0/1/2 为
2,635,468 / 2,631,979 / 2,629,370。Rust Fast 的 Fixed-candidate regret 总计 95,956 B，
p50/p95/worst 为 0 / 3,258 / 39,502 B，24/120 非零，None 在 81 张上实际获胜或并列。

Rust Fast 每图 p50/p95/worst 为 **16.108 / 106.185 / 121.793 ns/pixel**，pixel-weighted
12.081 ns/pixel；process p50 wall / CPU / RSS 为 **3.934 s / 3.870 s / 207.015 MiB**。
libwebp filtering=1 为 86.187 / 478.125 / 534.375 ns/pixel，pixel-weighted 133.423，process
p50 为 42.701 s / 42.230 s / 340.219 MiB。但 Rust 是内部 ALPH-only boundary，libwebp 是
public complete-WebP VP8 q75 boundary；这些速度只作 boundary-separated diagnostic，**不得作
跨库同边界速度结论**。Lossless owner share 的 all/native 92.7095% / 90.728706% 同样只是
inclusive owner diagnostic，不是可恢复 gap。

A19 冻结语义是：同一 invariant 在 conformance 与 native 上分别以 comparable size、wall 或
realistic recoverable-owner gap 的任一项达到 `>=10%`，且过 leave-largest、单图、类别支配与
3/5 strata 同方向门槛。最终选择 exact ALPH size 作为 comparable invariant。Native Rust Fast
对 libwebp filtering=1 为 1,804,131 对 1,274,713 B，机会 **29.344765%**；去最大贡献图仍
30.053037%，最大图贡献 16.965045%，五类同方向，均过对应门槛。但最终仍有三个冻结 gate 失败：

1. conformance 为 4,101,484 对 4,098,325 B，机会仅 **0.077021%**，低于 10%；
2. conformance leave-largest 仅 **0.906893%**，低于 10%；
3. native soft-shadow-gradient 贡献 445,671 / 529,418 B，即 **84.181309%**，超过 50% cap。

因此结论为 **no A19 direction**：native-only 的大差距不具备双语料与类别去支配鲁棒性，A18
不改变 production filter policy/default，也不把实验 instrumentation 合并到 main。最终报告、
限制、校验与机器判定分别在 `reports/alpha-real-image-corpus/README.md`、`LIMITATIONS.md`、
`VERIFICATION.md`、`raw/formal-final/gate.json` 与 `summary.json`；`SHA256SUMS` 的 723 项全部通过。
持久 cache `/Users/lance/Library/Caches/webp-alpha-real-image-corpus/v1/` 共约 4.7 GiB，其中已接受
screen outputs 约 1.4 GiB，corpus/output 二进制不进 Git。

保留的 invalidations 包括：初版/v2 corpus structural gate failure、Wikimedia 429 checkpoint/retry、
初版 full-screen PAM/Pillow reader failure、以 cumulative `RUSAGE_CHILDREN` 读取 RSS 的 formal、
main 从 `2234932d` 前进到 `29e7d6ef` 后的全部旧正式数据，以及在 P23/E48 协调窗口中停止的
`29e7d6ef` partial formal；它们均未进入 headline。稳定 toolchain 下 fmt、标准 workspace tests、
all-feature Clippy、Python/C checks 通过，284 个 unit tests 与 2 个 oracle tests 通过。唯一未完成的
all-feature integration test 依赖隔离 worktree 中不存在的外部 41/15 fixture；A18 没有借读根目录
untracked corpus 来绕过该边界，并已在 `VERIFICATION.md` 明示。
...
| licensed real-image dual-corpus A19 admission | A18 120-image licensed census + exact screen + 5-rotation formal on final `main@62d8afea` | native exact-ALPH opportunity **29.344765%** and leave-largest 30.053037%，but conformance only **0.077021%** / leave-largest 0.906893%，and soft-shadow-gradient contributes **84.181309%** of native gap | no A19 direction; retain the corpus as benchmark evidence, keep policy/default unchanged, and do not merge experiment instrumentation |
...
1. **Structured ALPH density:** Rust is still 15.14% above libwebp on the
   ...
   boundary. A16 has now also closed the exact-sized cursor sink direction:
   the remeasured current boundary is only 10.197% of ALPH, exact-u128 still
   regresses 0.625%, and u64/staged forms regress 9.854%–11.843%. Exact output
...
2. **Real-image evidence:** A18 has now frozen and measured a pinned, licensed
   120-image corpus: 84 native-raster primary images plus 36 separately labeled
   derived controls, with exact-alpha, alpha-cardinality, p50/p95/tail, CPU and
   peak-RSS evidence. The corpus remains a reusable benchmark boundary, but it
   admits **no A19 direction**: native exact-ALPH opportunity is 29.344765%,
   while conformance is only 0.077021% and 84.181309% of the native gap comes
   from soft-shadow-gradient. Future work needs a newly predeclared corpus or
   architecture question; it must not tune A18's classifier, source mix,
   threshold, or bucket boundaries after seeing these results.
