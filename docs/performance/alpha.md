...
| A18 | licensed real-image alpha corpus + filter-policy/owner census：先建立来源、许可、native/derived 与重复簇边界，再决定是否存在下一代架构 | `codex/alpha-real-image-corpus`；HEAD 尚等于 base | 精确创建并完成 intake 于 `401e48080e480600a9b8e94c8a2d8401e94b3a88` | [`b11f` worktree](</Users/lance/.codex/worktrees/b11f/webp>)；task `019f8a5a-cce8-7141-88c5-fc4014891dcb` | **已登记 / corpus acquisition 待放行**；v1 固定 120 张 = 84 native primary + 36 derived supplemental，五类各 24；计划报告 `reports/alpha-real-image-corpus/` | A18 census-only；不得产品化 filter policy，双 corpus 同 owner `>=10%` 且去除最大贡献图后仍过门才可提出 A19 | 本提交固定 base/task/worktree、来源许可规则、分层/去重/归一化、下载预算、指标矩阵与 A19 admission gates |
...
### A18 licensed real-image corpus 登记

A18 独立任务 `019f8a5a-cce8-7141-88c5-fc4014891dcb` 已从创建时最新本地
`main@401e48080e480600a9b8e94c8a2d8401e94b3a88` 精确建立；worktree、branch HEAD、
local main 与 merge-base 一致，目录 `/Users/lance/.codex/worktrees/b11f/webp` 干净，唯一分支
为 `codex/alpha-real-image-corpus`。Phase 0 只读审查了本文、现有 report schema、pins 与
primary-source 许可页面，尚未下载 corpus、修改文件或运行 benchmark。现有 corpus/oracle pins
仍为 `06ddd96e276c2c638a72d39d3c0f340afd61978c` /
`733c91e461c18cf1127c9ed0a80dccbcfed599d3`；CLIC `tfds:clic:1.0.0` 因预计下载
7.48 GiB、workspace 14.96 GiB 明确不进入 A18。

Corpus v1 在任何 encoder 结果可见前冻结为 **120 张**、五类各 24：translucent/photo
cutout、soft shadow/gradient、UI/icon、emoji/illustration、texture/sprite。84 张
`native-raster` 组成 real-image primary headline；36 张 vector render、程序化渲染或明确
派生物只进 derived 补充表。单 source 不超过三分之一、单 family 不超过 8 张，单图不得贡献
native cohort 超过 10% pixels；用 original/normalized/RGBA/alpha SHA-256 exact 去重，并以
RGB pHash、缩小 alpha mask、histogram/SSIM 标记近重复。Native raster 不缩放，超尺寸直接排除；
vector 固定 renderer/version/canvas/尺寸表并标为 `derived-vector-render`。

许可先于图像：Wikimedia 必须逐文件固定 file revision、original URL、作者、license template
与 attribution；Kenney CC0 packs 用页面版本/快照、ZIP SHA-256 与内含 LICENSE 内容寻址；
Material、Fluent Emoji、Twemoji 与 Khronos 只使用固定 commit，Khronos 还须逐模型审查。
GFDL-only、GPL-only、NC/ND、许可冲突、license laundering 风险、商标主体、可识别人物与只有
“free download”声明的来源默认排除。Git 只提交 acquisition/verify scripts、manifest、许可
快照/链接、报告和 raw metrics，不提交 corpus 二进制。缓存默认放在
`/Users/lance/Library/Caches/webp-alpha-real-image-corpus/v1/objects/sha256/`，允许
`WEBP_ALPHA_REAL_CORPUS_CACHE` 覆盖；预计原始 200–450 MiB、normalized 100–300 MiB、含
outputs/invalidated raw 的长期缓存 1–4 GiB。

冻结矩阵对每图记录尺寸、alpha cardinality/entropy/0-translucent-255 比例、四种 residual
entropy/zero ratio、来源/类别/native-derived buckets；Rust 比较 Fast、四个 Fixed filters 与
Best 的 exact ALPH bytes/bpp、完整 WebP bytes、method/filter/candidate/winner/tie，并分解
estimator、filter、palette/plain、LZ77、frequency/Huffman、token output 与 finalize owner。
Pinned libwebp 只在相同 public effort boundary 比较 none/fast/best；跨库 size 用 ALPH bytes/bpp，
speed 只用 whole public-API boundary，不伪称 RGB bitstream 或 standalone ALPH timing identical。
正式 timing 固定 5 rotations × 10 encodes/image，报告 wall/CPU/peak RSS、macro 与 pixel-weighted、
category/source/native-derived aggregate、p50/MAD/p95/worst；所有输出须 project decoder 与 pinned
`dwebp` exact-alpha，保留完整 hash manifests。

A18 本身 census-only。只有同一个 owner/invariant 在 41-file conformance 与 84-file native
primary 上都达到预声明 `>=10%` size、wall 或 recoverable-owner gap，最大单图贡献不超过 20%、
最大类别不超过 50%、去掉最大贡献图后仍 `>=10%`，且至少 3/5 类同方向，才允许提出独立 A19。
许可、exactness、来源集中度或 tail 任一 gate 失败，只能结论“无 A19 方向”；不得看结果后调整
classifier、threshold、来源配额或 bucket 边界。
...
2. **Real-image evidence:** add a pinned, licensed translucent PNG/WebP corpus
   ...
   fixtures. A18 is now registered to build this boundary as 84 licensed
   native-raster primary images plus 36 separately labeled derived controls;
   it is a census task, not permission to change filter policy. Any A19 must
   clear the same >=10% owner invariant on both conformance and native cohorts
   without single-image or single-category dominance.
