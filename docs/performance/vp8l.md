...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 50 个 VP8L/FDEC 实验、验证与产品迁移任务；当前没有运行中的实验。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
- E51 已冻结并实现这套边界，但 Phase-A 的 v1 控制 parser 自身失效；下一树先用真正的 RIFF chunk walker 修复 byte-scan/odd-padding/duplicate 语义并加入回归样本，再用新 binary、新输出目录完整重跑 229 图 exactness，不能续跑 E51 的五张 partial output。
