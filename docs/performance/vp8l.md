...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 52 个 VP8L/FDEC 实验、验证与产品迁移任务，另有 1 个实验运行中。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E54 | trusted prepared FDEC sidecar/cache hot path（P29） | `codex/fdec-trusted-sidecar-cache@d41c66e2`；设计与威胁模型待冻结 | `d41c66e2` | [planned artifacts](</Users/lance/.codex/worktrees/14f1/webp/experiments/fdec-trusted-sidecar-cache>)；[14f1](</Users/lance/.codex/worktrees/14f1/webp>)；task `019f8c39-9a0e-7193-8d66-180b2eef0284` | **运行中**：先做 alpha28 Phase-0 owner attribution；若可移除开销上限低于 practical 8% / fastest 6%，立即封口，不实现 prepared cache |
...
P29/E54 已从当时最新且 clean 的 `main@d41c66e252ed805a8641149b13e5b50499ae2732` 创建于独立 worktree [14f1](</Users/lance/.codex/worktrees/14f1/webp>)，并立即挂分支 `codex/fdec-trusted-sidecar-cache`，task `019f8c39-9a0e-7193-8d66-180b2eef0284`。它把标准 WebP 视为唯一分发资产，把 FDEC 定义为按 source SHA-256/schema/profile/build key 绑定、可驱逐的本地派生缓存；当前只读复核 P27/P28 并冻结设计/威胁模型，随后先量化 RIFF/fallback 扫描、目录解析、分配、Zstd context 与 CRC 的可移除上限。冷 admission、cache bytes 与 hot-hit latency 必须分开报告；它不能声称改善 embedded container 体积。
...
- 创建身份精确通过 `main@d41c66e252ed805a8641149b13e5b50499ae2732`，工作树 [14f1](</Users/lance/.codex/worktrees/14f1/webp>)、分支 `codex/fdec-trusted-sidecar-cache`，task `019f8c39-9a0e-7193-8d66-180b2eef0284`；初始状态 clean，HEAD/main/merge-base 三者相同。
- P27 alpha practical/fastest 候选中位为 102.375751/100.908752 ms，要跨过 50% 门还需分别省 6.259877/4.792878 ms，约为候选的 6.12%/4.75%。本轮先做 Phase-0 owner attribution；只有可移除 owner ceiling 至少达到 practical 8%、fastest 6%，才允许实现 prepared path。
- 若 Phase 0 通过，alpha28 的 same-binary warmup+F/R/F recovery 还必须达到 practical/fastest 至少 8%/6%、两档 0/28 回退，并保持相对标准 A 的完整路径不超过 0.5%；其后才进入 229/28 Phase A。当前没有性能结论，顶部纪录表不变。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。E53 进一步证明独立 alpha-plane 全 portfolio 只能再省 6,844/300,364 bytes，无法填平 3.07/2.88 MB 缺口。因此不再追 alpha 局部编码；P29/E54 正在独立 latest-main 树评估内容寻址、可驱逐的 trusted FDEC sidecar/cache，以及一次 admission 后去掉 hot-hit 每次 CRC/fallback 扫描的可行性。这是新产品类别，必须单独报告冷缺失、缓存字节、信任绑定与命中延迟，不能冒充嵌入式 FDEC 的体积改善。
