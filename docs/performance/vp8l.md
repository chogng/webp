...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 49 个 VP8L/FDEC 实验、验证与产品迁移任务，以及 1 个正在进行的实验；更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E51 | FDEC v2 row-group / streaming architecture（P26） | `codex/fdec-v2-row-groups`；HEAD 待实验收口 | `564adc27` | [f9db](</Users/lance/.codex/worktrees/f9db/webp>)；task `019f8b8a-df12-7b62-ab2c-185ecf0f910a`；证据目标 `experiments/fdec-v2-row-groups` | **进行中**：创建身份精确通过；独立 Zstd/LZ4 row groups、RGB/独立 alpha plane、融合逆变换/RGBA/CRC、有界单 group scratch，并在同一 latest-main 实现中重建 E16 控制与 E17 的 229 图 gate |
...
P26/E51 正在独立 worktree [f9db](</Users/lance/.codex/worktrees/f9db/webp>)、分支 `codex/fdec-v2-row-groups` 上运行，task `019f8b8a-df12-7b62-ab2c-185ecf0f910a`。HEAD/local main/merge-base 精确通过 `564adc27c6366f18fc51a8a475c28f473cfbfa1f`，且设计/provenance 在任何 corpus、rate 或 timing 输出前冻结。失败只归档；通过也不合入研究代码，而是从届时最新 committed main 新建最小产品迁移树。
