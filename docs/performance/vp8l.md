...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 48 个 VP8L/FDEC 实验、验证与产品迁移任务，以及 1 个正在进行的实验；更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
### 第一优先：fused entropy-map sufficient statistics + rank-sum

- E49 已把候选 table/canonical 构建全部删掉，并在 LowLatency 获得 **3.516% 且 0/41 回退**；这说明 exact metric 路线成立，但离 5% recovery 仍差 1.484pp。剩余每个候选都重复构造 synthetic `[0, group, 0, 0]` RGBA、residual/token `Vec`，再只取其 histogram：Compact/LowLatency 共 306/642 次 nested-map tokenization，正是下一条可独立删除且保持 wire 的成本。
- 下一独立树固定 A 为 E45 retained-full-plan，O 为 E49 rank-sum 加现有 generic nested-map collector，B 引入由 entropy-image 模块拥有的 streaming frequency sink。B 直接扫描 assignments，按现有 distance-one greedy 规则精确生成 literal/copy histogram（包括 4096 copy 分段、短尾、prefix/extra 与 distance-one），不 materialize synthetic RGBA、residuals 或 candidate tokens，再把五表 histogram 交给 E49 rank-sum。
- 该边界应面向“生成式像素源→精确频率”而不是硬编码某个 benchmark；P25 首个 consumer 是 entropy group map。候选仍只拥有 kind、`PlanParts` 和 metric；proposal/refinement/growth、tie/stop/fallback、最终 winner tokenization/table/wire、公开错误与 release A 均不变。不在同一树混入参数、classifier、并发或格式变化。
- 机制门必须逐 run length 0..=8193、跨 4096 边界、交替/长短 run、所有 group 值、map width/row 边界与 overflow 验证 B histogram/metric == O tokens/writer；102 图要求 A/O/B/public/P18 204/204、E/B/R/全部 Split exact，并将候选 synthetic RGBA/residual/token allocation 与 generic nested-map tokenization 降为 0。
- 唯一 recovery 继续要求 LowLatency ≥5% 且 0/41 回退、Compact aggregate ≤+0.5%。通过后固定融合 B，从最新 main 新建产品迁移树并跑双档 ≥50% screen、102×5 formal、双 decoder、Default identity、stable quality/resource/replay；失败则只归档证据，再转 Fast Representation v2。
