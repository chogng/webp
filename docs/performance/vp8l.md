...
- 根审计结论为 **Stage H PASS / 放行 Stage Q preregistration**。当前仍没有 codec timing、correctness 或性能结论，顶部表不变；下一提交必须先冻结 candidate、229/28 corpus/order、依赖、repeats、gates 与唯一 alpha28 evidence paths，之后才允许一次 eligibility。只有 alpha28 全门通过才进入 full229 Phase A；formal 仍需第二次根放行。
