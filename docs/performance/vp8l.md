...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 46 个 VP8L/FDEC 实验、验证与产品迁移任务，以及 1 个正在进行的实验；更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
### 第一优先：metric-only search / final-plan materialization

- E45 的产品搜索把 E、B、refined 和当前 growth winner 都保留为完整 `SpatialCostPlan`：除 `PlanParts`/RIFF metric 外，还长期持有 encoded prefix 与每组五张 `EntropyTables`；构建下一候选时旧 winner 仍在。P18 则让每个 exact plan 在取出 metric 后立即释放 prefix/tables，只保留 `PlanParts + metric`，最终选择完成后才重建一次可写 plan。两者输出相同，但 E45 LowLatency candidate 比 P18 慢 5.039%，而 control 只变化 -0.264%；336 次 growth 使该 lifetime 差异有明确放大路径。这是源码推导出的下一假设，尚未完成 A/B。
- 从归档后的 latest committed `main` 新建独立工作树。A 精确复建 E45 retained-full-plan search；B 只保留 compact metric record 与 `PlanParts`，每个候选 exact-cost 完成后立即 drop prefix/tables，最终 winner 只重建一次 `SpatialCostPlan` 并 selected-only write。禁止改算法、cost、tie、growth、rate 或输出。
- 同一最终 test binary 先证明 A/B/P18 两档 204/204 byte identity，并记录候选数、同时存活的 table/prefix bytes、分配次数、峰值 retained plan bytes 与最终重建次数。计数只在非 timing census 中开启，release B 不带 timer/counter/switch。
- 41 图 recovery screen 沿用 warmup+3 F/R/F，要求 B 相对 A 的 LowLatency aggregate 至少改善 3%、0/41 回退；Compact aggregate 不得慢超过 1%。若通过，再把 B 固化并从头重跑 E45 的双档 ≥50% product screen、102×5 formal、正确性、Default identity、stable quality、资源/size 与隔离 replay；若失败则立即归档，不在同一树追加 sparse representation 或其他调参。
