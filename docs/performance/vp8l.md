...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 51 个 VP8L/FDEC 实验、验证与产品迁移任务。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。下一实验必须将范围缩到 alpha-plane owner portfolio：优先评估 constant/opaque、binary/tiny-palette bitpack、RLE、vertical/row/gradient predictor 与有界 Zstd/LZ4 选择；目标是再恢复约 6.1%/4.75% 解码时间和约 18%/17% private bytes，同时保持 opaque/RGB 路径字节与性能不变。
