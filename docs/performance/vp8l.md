...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 43 个 VP8L/FDEC 实验、验证与产品迁移任务；更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断或通过实验 gate 后另建 latest-main 产品迁移树，两棵树分别登记，避免把诊断提交误认成产品 HEAD。
...
- 从归档后的最新 committed `main` 新建独立工作树，不整体 cherry-pick、merge 或 rebase P18。先阅读 P18 的设计、实现差异和证据，再在当前生产边界中手工重建最小 planner 分工；research feature、trace、census、宽泛 benchmark hook、失效 runner 与 raw 输出不得进入生产代码。
- 产品语义必须保持 E43 的固定 profile 规则和输出流：Compact 只做 E/B + 一次 reassignment，LowLatency 才做 deterministic capacity growth；两档共享 prepare、exact counters/cost、single fallback 与 selected-only writer。Default、公开 API、metadata、animation 与错误行为不变；禁止图 ID、classifier、参数搜索和并发。
- 先逐图证明产品 Compact/LowLatency 与 P18 candidate byte-identical，并以同一最新-main binary 建立产品 control。41 图 screen 要求两档各至少改善 50%、0/41 编码回退、rate aggregate 不劣于 control且单图不超 +2%、Rust/pinned C decode 回退不超 1%、RSS 增量不超 64 MiB/5%，所有流完整 RGBA exact。
- screen 全过后跑锁定 102 图五轮 formal，要求两档各至少改善 50%、0/102 逐图中位回退、绝对中位不高于 7.1/6.9 s；随后完成 102 图多布局双 decoder、Default feature/no-feature byte identity、stable workspace/Clippy/fmt/docs、依赖/API/资源审计和从仓库根启动的一键复现。
- 模块按数据所有权拆分并保持依赖单向，新增生产模块目标低于 500 行；若 latest-main 合法变化使输出或基线改变，必须报告实际差异并重新判门，不能用 P18 的旧数字替代产品测量。只有这棵产品树全部过门并经主任务独立审计后，才更新顶部产品纪录并迁入 main。
