...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 45 个 VP8L/FDEC 实验、验证与产品迁移任务；更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
### 第一优先：sparse exact-histogram merge recovery

- 从归档后的最新 committed `main` 新建独立工作树，显式挂分支。以 E45 产品输出为固定语义，只允许一个算法差异：合并 block histogram 时对零 counter 直接跳过，不执行无效 checked add；禁止改 clustering、growth、tie、rate、writer、profile 路由或 gate，也不允许按图/尺寸启用。
- 在首次 headline 前冻结设计，并在同一最终 test binary 内提供 test-only 对称 A/B：A 为 E45 无条件 1,049-slot checked-add merge，B 为 zero-eliding merge。先统计每阶段 slot visits、nonzero adds、zero-elision ratio 与 LowLatency growth/rebuild 归属；B 必须两档 204/204 与 A/P18 byte-identical。
- 41 图 warmup+3 交错 recovery screen 要求 B 相对 A 的 LowLatency aggregate encode 至少改善 3%、0/41 回退；Compact 不得慢超过 1%，两档输出必须逐字节一致。3% 同时覆盖 E45 的 1.810pp 缺口和合理噪声余量；未达到就拒绝，不以单次调度异常补考。
- recovery gate 通过后才把 B 固化为产品实现，并从头重跑 E45 的最终-binary Phase A、双档 ≥50% screen、102×5 formal、双 decoder、Default identity、完整 stable quality、资源/size 与隔离 replay。任何旧 P18/P20 性能数字只能作历史对照，不能替代新树测量。
- 若 sparse zero-elision 仍不足，归档后再评估以 sparse index/run representation 让 histogram 所有权本身只遍历非零项；不能在同一实验看结果后追加第二算法，也不能靠并发越过单线程门槛。
