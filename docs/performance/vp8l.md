...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 51 个 VP8L/FDEC 实验、验证与产品迁移任务，以及 1 个运行中的 alpha-plane 恢复实验。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E53 | FDEC v2 alpha-plane owner portfolio recovery（P28） | `codex/fdec-v2-alpha-plane-portfolio@a5151ab3`（创建身份） | `a5151ab3` | [676f](</Users/lance/.codex/worktrees/676f/webp>)；task `019f8c1f-4406-7b13-8021-90235de8e45a` | **运行中，未计时**：先做 28 张 alpha 的 owner ceiling；只有能同时追回 practical/fastest 的 3,074,837/3,183,870 private bytes 与约 6.12%/4.75% 解码时间，才实现有界逐组 portfolio，否则直接负报告封口 |
...
P28/E53 已从当时 exact latest `main@a5151ab3b4a16ee84709e7fb08fcb005dff6ee51` 建于独立 worktree [676f](</Users/lance/.codex/worktrees/676f/webp>)，并立即挂到 `codex/fdec-v2-alpha-plane-portfolio`，task `019f8c1f-4406-7b13-8021-90235de8e45a`；HEAD/main/merge-base 与 clean status 均已核对。本树先做 owner ceiling，如果 alpha 自身的可消除 bytes/time 不足以同时跨过两档硬门，则不实现、不计时；即使研究 gate 全过，也必须再从届时 latest main 新建产品迁移树。
...
### E53：FDEC v2 alpha-plane owner portfolio recovery（运行中）
...
- 创建身份为 `main@a5151ab3b4a16ee84709e7fb08fcb005dff6ee51`，工作树 [676f](</Users/lance/.codex/worktrees/676f/webp>)、分支 `codex/fdec-v2-alpha-plane-portfolio`，task `019f8c1f-4406-7b13-8021-90235de8e45a`；启动时 HEAD/main/merge-base 精确相等且 clean。
- Phase 0 先锁定上限：practical/fastest 的 alpha private bytes 必须从 17,088,816/18,731,020 降到不高于 **14,013,979/15,547,150**，即再减 3,074,837/3,183,870；alpha time ratio 必须从 0.532564/0.524933 降到不高于 0.5，即候选还须约快 6.12%/4.75%。只有 oracle lower bound 和可实现 upper bound 都支持同时过门，才放行代码。
- 后续仍必须重跑固定 229/28 exactness、双 decoder、checked-RIFF mutation/limits、不高于 1 MiB scratch 和零整帧 residual；201 opaque candidates 应与 E52 字节一致。测量必须有界加载，不允许再用 multi-GiB 全 corpus preload 伪装 decoder RSS。Phase A/alpha screen 全过后仍须暂停等根审计，未放行不得 formal。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。P28/E53 已在 latest-main 独立树中开始 alpha-plane owner ceiling；它只在理论和可实现上限都能同时恢复约 6.12%/4.75% 解码时间与约 18%/17% private bytes 时才实现 portfolio，并必须保持 opaque/RGB 路径字节与性能不变。
