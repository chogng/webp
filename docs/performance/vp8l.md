...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 54 个 VP8L/FDEC 实验、验证与产品迁移任务，另有 1 个 qualification 的非计时 harness 阶段运行中。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E56 | atomic-evidence trusted sidecar full qualification（P31） | `codex/fdec-sidecar-full-qualification@35c768af`；设计待提交 | `35c768af` | [planned artifacts](</Users/lance/.codex/worktrees/58a8/webp/experiments/fdec-sidecar-full-qualification>)；[58a8](</Users/lance/.codex/worktrees/58a8/webp>)；task `019f8c79-60e9-7c51-915c-bf5aed6d394a` | **Stage H 运行中、尚无 timing**：先用 stub/fault injection 证明 parent-only 单终态、staging→final 原子发布与 manifest finalization；根审计放行后才可创建全新的 alpha28/full229 qualification evidence |
...
P31/E56 已从当时最新且 clean 的 `main@35c768af9d868403945d62f7a45cb16417d7197e` 创建于独立 worktree [58a8](</Users/lance/.codex/worktrees/58a8/webp>)，并立即挂分支 `codex/fdec-sidecar-full-qualification`，task `019f8c79-60e9-7c51-915c-bf5aed6d394a`。当前只做非计时 Stage H：parent runner 独占终态，数据先进入唯一 staging，stub/fault-injection 覆盖成功、各失败点、已有 root 与中断，校验完成后才原子发布 final root。Stage H 提交后必须暂停由根任务审计；未获放行不得创建任何 codec timing evidence。
...
- Stage H 必须先冻结 `QUALIFICATION_DESIGN.md` 与 `HARNESS_STATE_MACHINE.md`，再用不运行 codec 的 stub/fault injection 证明 success、benchmark/resource/postprocess/manifest/finalize failure、已有 root 与 signal/interruption 均只产生一个可判定终态；禁止 success+failure 共存，失败 staging 不能冒充可消费 final-success root。
- 当前没有 timing、correctness 或性能结论，顶部表不变。Stage H 完成后先提交并暂停；只有根审计明确放行，才允许用新的唯一 evidence root 执行一次 alpha28 eligibility，过门后再进入 full229 Phase A，formal 仍需第二次根放行。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。E53 证明独立 alpha-plane 全 portfolio 无法填平体积缺口；E54 则证明 trusted prepared sidecar 的 hot B 在 alpha28 相对 same-binary A 快 87.023%/89.178%，且相对 fresh O 还能快 31.340%/35.331%。E55 的固定批量 recovery 在数值上把两档逐图失败降到 0/28，并保持 B/O 0.697625/0.666013，但由于唯一 evidence root 的终态矛盾只能封为 invalid-negative。下一树先把 harness 终态互斥、staging→final 原子发布与 manifest finalization 做成可单测不变量，再把该架构作为全新 full229 qualification；这仍是独立产品类别，必须分开报告 source bytes、cache bytes、cold admission 与 hot-hit latency。
