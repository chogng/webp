...
### 第一优先：allocation-free rank-sum exact Huffman cost

- E48 已证明候选序列化、prefix 和跨组 table 生命周期不是剩余缺口：这些对象全部删除后 LowLatency 只快 0.818%。真正未减少的是 Compact/LowLatency 每 102 图 **35,115 / 43,905** 次 adaptive-table 构建；每次当前都分配 ranked、lengths、canonical symbols 和 codes，并执行 frequency 排序与 canonical 排序，而候选 metric 最终只需要 code width 加权和。
- 当前 deterministic balanced table 对 `k>=2` 个非零 symbol 只有相邻两种长度。令 `L=floor(log2(k))`、`B=2^L`、`m=2(k-B)`，则最小的 `m` 个 frequency 使用 `L+1`，其余使用 `L`；精确 payload bits 可直接写成 `L*sum(f) + sum(m 个最小非零 f)`。`k=0/1` 的 data-symbol cost 都是 0，header 仍为 `63+4N`；frequency 相同的 tie 不影响加权和。
- 下一独立树固定 A 为 E45 retained-full-plan，并保留 E48 的逐位解析 oracle 作为非计时对照；B 用固定上界 stack scratch 加 `select_nth_unstable`（或经证据证明更快的等价无分配选择）直接算 rank-sum，不构造候选 `EncodingTable`、lengths/codes `Vec` 或 canonical code。nested-map tokenization、copy extras、proposal/refinement/growth、tie/fallback、最终 winner 的真实 table/wire 均不变。
- 先对所有合法 alphabet 长度、empty/one/sparse/dense/tied/max/overflow-adjacent 与随机确定性 frequency 矩阵证明 rank-sum、E48 oracle、真实 writer 三者 bits 相等；102 图必须覆盖 E/B/R/全部 Split，候选 adaptive-table builds 与 heap allocations 都降为 0，且 A/B/public/P18 逐字节一致。任何 metric、selector、错误类别或 wire 漂移立即拒绝。
- 唯一 41 图 recovery 仍为预载、一次 warmup、三轮 F/R/F：LowLatency 相对 A 至少快 5%、0/41 回退，Compact aggregate 不慢于 +0.5% 并报告所有逐图回退。通过后固定 B，再从头跑双档 ≥50% product screen、102×5 formal、双 decoder、Default identity、stable quality、资源/size 与隔离 replay；失败则归档，不在同一树混入 nested-map specialization、参数或并发。
