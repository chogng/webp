...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 53 个 VP8L/FDEC 实验、验证与产品迁移任务，另有 1 个 recovery 运行中。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E55 | trusted sidecar calibrated tiny-image recovery（P30） | `codex/fdec-trusted-sidecar-calibrated-recovery@c4e0d3f0`；冻结设计待提交 | `c4e0d3f0` | [planned artifacts](</Users/lance/.codex/worktrees/3207/webp/experiments/fdec-trusted-sidecar-calibrated-recovery>)；[3207](</Users/lance/.codex/worktrees/3207/webp>)；task `019f8c5e-b1c6-75f3-81dd-accca2510de5` | **运行中**：只修正亚微秒计时量化；按 RGBA bytes 固定 inner repeats、5 轮 paired AB/BA。alpha28 全过才允许完整 229 图 Phase A |
...
P30/E55 已从当时最新且 clean 的 `main@c4e0d3f05a4f854149eafaccf7a0a5ddc8b7be9c` 创建于独立 worktree [3207](</Users/lance/.codex/worktrees/3207/webp>)，并立即挂分支 `codex/fdec-trusted-sidecar-calibrated-recovery`，task `019f8c5e-b1c6-75f3-81dd-accca2510de5`。它不得改写 P29，只允许在看到任何新 timing 前按输出 bytes 冻结 inner-repeat 公式，使每个 tiny-image 样本执行足够多次完整 decode；同图 O/B/A 使用相同 repeats，输出每次仍新分配、完整写出并 black-box。alpha28 经过五轮 paired AB/BA 且逐图稳定过门后，才可手工重建完整 229 图 sidecar、mutation、资源与 cache-policy Phase A。
...
- 创建身份精确通过 `main@c4e0d3f05a4f854149eafaccf7a0a5ddc8b7be9c`，工作树 [3207](</Users/lance/.codex/worktrees/3207/webp>)、分支 `codex/fdec-trusted-sidecar-calibrated-recovery`，task `019f8c5e-b1c6-75f3-81dd-accca2510de5`；初始状态 clean，HEAD/main/merge-base 三者相同。
- alpha28 recovery 要同时满足 aggregate B/O 0.92/0.94、两档 B/A 0.5、逐图 paired median 0/28 回退，并且每图至少 4/5 paired batches 由 B 胜出。任一失败立即封口；全过才进入 229 图 Phase A。当前没有性能结论，顶部表不变。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。E53 证明独立 alpha-plane 全 portfolio 无法填平体积缺口；E54 则证明 trusted prepared sidecar 的 hot B 在 alpha28 相对 same-binary A 快 87.023%/89.178%，且相对 fresh O 还能快 31.340%/35.331%。E54 只因 16×16 图约 125 ns 的三样本中位回退按硬门拒绝。P30/E55 正在 latest-main 树用预先固定的 size-calibrated inner repetitions 做唯一 recovery；若过门才进入 229 图、mutation、资源与 cache-policy Phase A。这仍是独立产品类别，必须分开报告 source bytes、cache bytes、cold admission 与 hot-hit latency。
