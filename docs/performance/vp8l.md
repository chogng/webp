...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 50 个 VP8L/FDEC 实验、验证与产品迁移任务，以及 1 个运行中的修正版实验。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
| E52 | FDEC v2 checked-RIFF corrected audit（P27） | `codex/fdec-v2-row-groups-corrected@020450d3`；设计 `7b3ebe94`；机制 `fce2515f`；结构 `fa8426fd`；provenance `020450d3` | `26a6d2fb` | [frozen design](</Users/lance/.codex/worktrees/e392/webp/experiments/fdec-v2-row-groups-corrected/DESIGN.md>)；[e392](</Users/lance/.codex/worktrees/e392/webp>)；task `019f8ba2-c761-77a2-bfcb-1a4a5c9002e7` | **运行中，未计时**：只把 v1 O 与 v2 B 的 container association 统一为 checked RIFF chunk walker；先完整重跑固定 229/28 Phase A，全部通过且根审计放行后才允许唯一 warmup+formal |
...
P27/E52 已从创建时最新且 clean 的 `main@26a6d2fbd2264e9bf2237cad3951748c09f278b8` 建于独立 worktree [e392](</Users/lance/.codex/worktrees/e392/webp>)，并立即挂到 `codex/fdec-v2-row-groups-corrected`，task `019f8ba2-c761-77a2-bfcb-1a4a5c9002e7`。设计 `7b3ebe94` 冻结后只选择性移植 P26 已审查的机制/结构，provenance 修正为 `020450d3`；当前只运行非计时 Phase A，尚无性能、rate 或产品结论。
...
### E52：FDEC v2 checked-RIFF corrected audit（运行中）
...
- 创建身份精确通过 `main@26a6d2fbd2264e9bf2237cad3951748c09f278b8`，工作树 [e392](</Users/lance/.codex/worktrees/e392/webp>) 立即挂分支 `codex/fdec-v2-row-groups-corrected`。冻结设计 `7b3ebe94` 后，机制/build/结构分别作为 `fce2515f`/`287f7c3e`/`fa8426fd` 重建；P27 与根任务双 ID 在 `020450d3` 修正并提交。
- Phase A 必须重新解析并逐项 hash 固定 E17 ledger `3e9e0c0b…efe2f878` 的 229 图/28 alpha，使用新 release binary 与不可覆盖的外部 ordered shards；P26 的 `/private/tmp/p26-fdec-v2-phase-a-3f9b6499` 只保留为 invalid evidence，禁止续跑或复用。
- 放行前分母包括：201 opaque 的 O practical/fastest、229 图两档 B、28 alpha 私有选择、402 O + 458 B pinned `dwebp` exact、fallback 除 RIFF length 外字节一致、RIFF 边界回归、独立 group/row-range/one-scratch/output-visit/CRC census。任何一项失败即无 timing；全部通过后仍须暂停，由根任务独立审计并释放同机唯一 warmup+五轮交错测量。顶部纪录表在 formal 通过前不变。
...
- E51 已冻结并实现这套边界，但 Phase-A 的 v1 控制 parser 自身失效。E52 正在独立 latest-main 树中以 checked RIFF chunk walker 修复 byte-scan/odd-padding/duplicate 语义，并用新 binary、新输出目录完整重跑 229 图 exactness；它不能续跑 E51 的五张 partial output，且通过根审计前不得计时。
