...
- same-input performance：分别从 Phase C 与 Phase D committed source 的 fresh archive
  独立 release 构建，
  对同一 CLIC-102 / m0+m3+m6 / 306 streams / 3,022,297,644 RGBA bytes 做三轮交替。
  Phase C 为 `15,163.347 / 15,106.852 / 15,204.596 ms`，Phase D 为
  `14,857.828 / 14,879.695 / 14,866.429 ms`；中位 **15,163.347 →
  14,866.429 ms（-1.958%）**，每轮 checksum 均为 `997056`。
- evidence correction：本阶段最初登记的 -12.634% 使用了一个未能由 committed source
  重建的 stale current binary（659,600 B）；同一 artifact 也污染了当时 Rust/pinned-C
  数值，现永久作废且不进入任何 headline。`main@30aa70eb65d42222dde586818315ae4f02a21cd8`
  的独立 archive 可重复构建为 628,800 B；上述三轮仅使用 fresh archive binaries。
  pinned libwebp 的 fresh same-run 对比留到后续跨域产品验收，不从失效运行拼接。
...
- resource：宿主边界 `/usr/bin/time -l`、相同 306-stream process policy 下，
  Phase C/Phase D fresh archive 峰值 RSS 为 **905,658,368 / 869,482,496 B
  （-36,175,872 B，-3.994%）**。release `decode_bench` 为
  628,704 → 628,800 B，**+96 B（+0.015%）**。
...
### Phase E：compact Huffman view 与 scalar kernel family

Date: 2026-07-23。Base:
`main@f8fcf80aba17954a792b93b6b5fc75750ba23948`。Branch/worktree: 本地 `main`
根工作区，未创建 worktree。Host: Apple ARM64，stable
`rustc 1.97.1 (8bab26f4f 2026-07-14)` / LLVM 22.1.6。

当前产品原本已只有一份长期 pixel table：`FastHuffmanTable` 在 single、packed
root/secondary 与 rare strict fallback 三个 scalar family 中择一；header validation
使用的通用 canonical table 在 `into_fast` 后释放，不是长期双表。literal-heavy 路径
使用一次 bit-register snapshot 解四 channel，copy/cache 与 fallback 保留严格
single-symbol/EOF/WorkBudget 语义。

本切片把构建后不可变、且容量等于长度的 packed root/secondary 从 `Vec<u16>` 收紧为
`Box<[u16]>`。entry bits、root mask、secondary offset 和 lookup 完全不变；64-bit
`FastHuffmanTable` handle 从 56 B 降至 40 B，五表 group 少 80 B handle working set，
root/secondary 仍各只有一份 allocation。没有额外 pair table、transducer 或长期
generic table。

- correctness：decoder VP8L 136 passed / 3 ignored；workspace、workspace
  `clippy --all-targets -D warnings`、158-file fixture identity、上游 lossless、
  pinned-libwebp round-trip、animation state/ALPH、cache/copy/palette/tiny、
  malformed、逐 byte truncation、allocation limits 与 WorkBudget 全过。
- same-input performance：fresh Phase D archive 与当前 fresh release 对同一
  CLIC-102 / m0+m3+m6 / 306 streams / 3,022,297,644 RGBA bytes 三轮交替。Phase D
  `14,904.641 / 14,952.645 / 14,782.139 ms`；当前
  `14,634.546 / 14,753.442 / 14,720.128 ms`；中位 **14,904.641 →
  14,720.128 ms（-1.238%）**，checksum 均为 `997056`。
- pinned C：同一 locked corpus/oracle commit
  `733c91e461c18cf1127c9ed0a80dccbcfed599d3` 的 aggregate 为 14,091.846 ms；
  当前 Rust fresh 中位仍慢 4.458%。该差距诚实保留，不用不同输入或 private cache
  掩盖。
- resources：release `decode_bench` 628,800 → 628,736 B（-64 B）。process RSS
  单次为 871,317,504 B；相对 Phase D 的 869,482,496 B 高 0.211%，远大于 handle
  模型节省且属于进程级测量噪声，因此只登记为 neutral/no proven RSS win，不宣称下降。
- SIMD：ARM64 variable-length Huffman lookup 是串行 bit dependency，predictor 也依赖
  已重建邻居。显式 NEON 会要求 unsafe/双路径且本轮没有可隔离的收益证据；按路线图
  “若确有收益”条件不添加，保留 scalar 与 LLVM auto-vectorization。x86-64 没有本机
  真实执行能力，列入最终外部验收而不伪造。
