...
根任务：`019f8321-035e-7211-8f53-987e18891c8c`。下表覆盖该任务已经收口的 53 个 VP8L/FDEC 实验、验证与产品迁移任务。更早的 `vp8l-huffman-paper-feasibility` 属于另一根任务，未混入这份计数。一个假设若因系统中断、创建期间 main 前进或通过实验 gate 后另建 latest-main 产品迁移树，各棵树分别登记，避免把失效 preflight 或诊断提交误认成产品 HEAD。
...
- E52 已用 checked RIFF 语义完成 229/28 exactness 与唯一 formal：完整路径两档快 51.898%/56.772%，但 alpha 速度仅 46.744%/47.507%、private bytes 仅减 2.447%/3.617%。E53 证明独立 alpha-plane 全 portfolio 无法填平体积缺口；E54 则证明 trusted prepared sidecar 的 hot B 在 alpha28 相对 same-binary A 快 87.023%/89.178%，且相对 fresh O 还能快 31.340%/35.331%。E54 只因 16×16 图约 125 ns 的三样本中位回退按硬门拒绝。下一棵 latest-main recovery 应在看任何 timing 前冻结 per-image calibrated inner repetitions，使每个样本至少覆盖固定毫秒窗口，再完整重跑 alpha28；若过门才进入 229 图、mutation、资源与 cache-policy Phase A。这仍是独立产品类别，必须分开报告 source bytes、cache bytes、cold admission 与 hot-hit latency。
