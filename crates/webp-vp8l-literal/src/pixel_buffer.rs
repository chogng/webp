use crate::inverse_predictor::inverse_predictor_argb_to_rgba;
use crate::inverse_predictor::inverse_predictor_rgba;
use crate::pixel::extend_rgba_from_argb;
...
pub(super) enum PixelBuffer {
    Argb(Vec<u32>),
    Rgba(Vec<u8>),
}

impl PixelBuffer {
    pub(super) fn argb_mut(&mut self) -> Result<&mut Vec<u32>, DecodeError> {
    ...
    pub(super) fn inverse_predictor(
        &mut self,
    ...
    pub(super) fn into_rgba(mut self, expected_rgba_len: usize) -> Result<Vec<u8>, DecodeError> {
        self.rgba_mut()?;
        match self {
