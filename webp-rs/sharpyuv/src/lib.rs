...
pub enum SharpYuvError {
    InvalidDimensions,
    InvalidPlaneLayout,
    AllocationFailed,
}

pub struct SharpYuvPlanes<'a> {
    pub y_stride: usize,
    pub uv_stride: usize,
    pub y: &'a mut [u8],
    pub u: &'a mut [u8],
...
pub fn convert_rgba_to_yuv420(
    ...
    let width = usize::try_from(width).map_err(|_| SharpYuvError::InvalidDimensions)?;
    let height = usize::try_from(height).map_err(|_| SharpYuvError::InvalidDimensions)?;
    let padded_width = width
        .checked_add(1)
        .ok_or(SharpYuvError::AllocationFailed)?
        & !1;
    ...
    {
        return Err(SharpYuvError::InvalidPlaneLayout);
    }
...
impl State {
    fn new(width: usize, height: usize) -> Result<Self, SharpYuvError> {
        let pixels = width
            .checked_mul(height)
            .ok_or(SharpYuvError::AllocationFailed)?;
        let uv = pixels
    ...
    fn refine(&mut self) -> Result<(), SharpYuvError> {
    ...
    fn convert(&self, width: usize, height: usize) -> Result<VisibleYuv, SharpYuvError> {
...
fn zeroed<T: Clone + Default>(len: usize) -> Result<Vec<T>, SharpYuvError> {
    let mut values = Vec::new();
    values
        .try_reserve_exact(len)
        .map_err(|_| SharpYuvError::AllocationFailed)?;
