...
| A07 | ALPH exact color cache：复用既有 greedy token 流，对标准 cache bits 1..11 做逐像素状态模拟与完整 bitstream 计价 | `codex/alpha-exact-color-cache`；进行中 | worktree 创建并核验目标为 `5362912a23a39175758796e07f45af3ee79143b1`；正式数据前须重放到本登记提交后的最新 `main` | [`40fa` worktree](</Users/lance/.codex/worktrees/40fa/webp>)；task `019f8825-6465-74c3-a880-ea07b1870c26` | **Phase A**：逐字节复现 no-cache，枚举 1..11 bits；完整计入 cache header、扩展 green alphabet、table RLE、token/extra bits，并分 40 structured/all-41/4 real/11 synthetic 报告 | 未决；structured 净 ALPH 必须 >=10%，56-file exact fallback 零膨胀；过门后才实现并测 encode/decode | 已登记；不增加第二套 LZ parse，不以 hit-count/Shannon 估算代替实际最终 bytes |
| A08 | optimal length-limited Huffman：量化当前超长树触发整树 `balanced_lengths` 的损失，以 package-merge 精确替代并按完整 payload 选择 | `codex/alpha-length-limited-huffman`；进行中 | worktree 创建目标为当时最新 `8574b4ef0731902d2f4fcfa4c829c661136a8c17`；正式数据前须重放到本登记提交后的最新 `main` | [`e754` worktree](</Users/lance/.codex/worktrees/e754/webp>)；task `019f8836-b206-7750-9b7e-8de995eeba06` | **Phase A**：分别 census 主表 limit=15 与 code-length 表 limit=7 的 fallback；用小规模 oracle 验证 package-merge，再实际序列化完整 ALPH 与 56-file exact fallback | 未决；structured 净 ALPH 必须 >=10%，否则只保留 owner/analyzer 负证据 | 已登记；不把 weighted/Shannon bits 当最终收益，必须计入 table RLE/header、token、extra bits 与 padding |
...
1. **Structured ALPH density:** Rust is still 15.14% above libwebp on the
   ...
   table/map cost. Further work should test the remaining standard color-cache
   mechanism on the already-owned greedy token stream, pricing cache headers,
   the expanded green alphabet, table RLE and every cache substitution exactly,
   rather than relaxing the evidence boundary or retuning the RowRLE/spatial
   portfolios.
