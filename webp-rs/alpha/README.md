...
| A06 | ALPH spatial entropy groups：不增加第二套 LZ parse，以标准 meta-Huffman map 聚类局部 green/distance 统计 | `codex/alpha-spatial-entropy-groups`；进行中 | 创建并核验于 `39feb8dd07b3739d5d9eb63dedeeb4e857ec3b28`；任何正式数据前须重放到届时最新 `main` | [`d454` worktree](</Users/lance/.codex/worktrees/d454/webp>)；task `019f87ea-a348-7062-833f-e32f04732803` | **Phase A**：构建 exact single-group baseline、independent-tile 下界与 <=64-group deterministic clustering；完整计入五表、nested group-map/header/data、extra bits，并分 40 structured/all-41/4 real/11 synthetic 报告 | 未决；structured 净 ALPH 必须 >=10%，过门后才实现并测 decode 回退 | 已登记；读取 E20/E23 仅作先验，不复用旧错误基线，最终回填 analyzer/raw、最新 base、HEAD 和决定 |
