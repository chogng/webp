...
use crate::encode_huffman::table_from_codes_for_test;
...
fn reference_packet_and_packed_traversals_match_with_cache_or_replay() {
    ...
    let green = table_from_codes_for_test(vec![(0, 0); 280]);
    let distance = table_from_codes_for_test(vec![(0, 0); 40]);
    let mut token_table = crate::encode_lz77::MatchTable::allocate(samples.len()).unwrap();
    let mut cached = Vec::new();
    crate::encode_lz77::walk(&samples, &mut token_table, |token| {
        cached.push(crate::encode_lz77::pack(token));
        Ok::<_, ()>(())
    })
    .unwrap();
    ...
    for cache in [None, Some(cached.as_slice())] {
        let render = |variant| {
            let mut prefix = BitWriter::new();
            prefix.write_bits(5, 3).unwrap();
            let mut match_table = crate::encode_lz77::MatchTable::allocate(samples.len()).unwrap();
