//! Decoding primitives for WebP's `ALPH` chunk.
//!
...
fn decode_lossless_samples(
    ...
    let image = webp_vp8l_literal::decode_vp8l(&encoded, &vp8l_limits)?;
...
fn vp8l_header(width: u32, height: u32) -> Result<Vec<u8>, DecodeError> {
    ...
    bits.write_bits(0x2f, 8)?;
    bits.write_bits(width - 1, 14)?;
    bits.write_bits(height - 1, 14)?;
    bits.write_bits(0, 1)?; // alpha is represented by the outer ALPH chunk.
    bits.write_bits(0, 3)?; // VP8L version.
    Ok(bits.into_bytes())
}
