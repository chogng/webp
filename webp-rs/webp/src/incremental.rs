...
impl IncrementalDecoder {
    ...
    fn decode_alpha(&self, width: u32, height: u32) -> Result<Option<Vec<u8>>, DecodeError> {
        ...
        match self.parser.alpha {
            Some(alpha) if alpha.payload_complete(self.bytes.len()) => crate::alpha::decode(
...
fn selected_codec(container: &webp_container::Container<'_>) -> Result<CodecKind, DecodeError> {
    if container
        .chunks()
        .iter()
...
fn validate_static_layout(
    container: &webp_container::Container<'_>,
    selected: CodecKind,
) -> Result<(), DecodeError> {
