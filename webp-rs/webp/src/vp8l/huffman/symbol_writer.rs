...
pub(crate) enum WireWriteError {
    SizeOverflow,
    AllocationFailed,
}

pub(crate) struct EncodingTable {
    pub(crate) codes: Vec<(u32, u8)>,
}

pub(crate) fn canonical_table(lengths: &[u8]) -> Result<EncodingTable, WireWriteError> {
...
pub(crate) fn write_table_symbol(
...
pub(crate) fn table_wire_symbol(
    table: &EncodingTable,
    symbol: usize,
...
#[cfg(test)]
pub(crate) fn table_from_codes_for_test(codes: Vec<(u32, u8)>) -> EncodingTable {
    EncodingTable { codes }
}

pub(crate) fn write_simple_table(writer: &mut BitWriter, symbol: u8) -> Result<(), WireWriteError> {
