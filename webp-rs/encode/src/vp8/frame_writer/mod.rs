...
use webp_dsp::predict_intra16_macroblock;
...
pub fn encode_dc_predicted_key_frame_with_quantizer(
    ...
    let (first_partition, token_partition) =
        if let Some(skip_probability) = skip_probability(&blocks) {
...
> {
    ...
    let pixels =
        reconstruct_intra_macroblock(block, &dc_macroblock_residuals(coefficients), matrix, edges)
            ...
            for (band, contexts) in bands.iter().enumerate() {
                for (context, nodes) in contexts.iter().enumerate() {
                    for (node, counts) in nodes.iter().enumerate() {
                        ...
                        let update =
                            COEFFICIENT_UPDATE_PROBABILITIES[coefficient_type][band][context][node];
...
fn coefficient_cost(values: impl Iterator<Item = i16>) -> u64 {
    values.map(|value| u64::from(value.unsigned_abs())).sum()
}
...
fn quantize_chroma_plane(
    ...
    std::array::from_fn(|block| {
        ...
        quantize_block(
            forward_dct_4x4_i32(residual_block(
                plane, stride, block_x, block_y, prediction, 8,
            )),
            matrix.uv_dc,
            matrix.uv_ac,
        )
...
fn write_coefficients(
    ...
    result.map_err(|error| match error {
        ...
        CoefficientEncodeError::InvalidParameter
        | CoefficientEncodeError::CoefficientOutOfRange => Vp8EncodeError::FirstPartitionTooLarge,
    })
}
