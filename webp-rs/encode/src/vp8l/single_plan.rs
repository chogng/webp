...
use super::token_stream::TokenStatistics;
use super::{
...
impl SinglePlan {
    pub(crate) fn build(statistics: &TokenStatistics) -> Result<Self, EncodeError> {
        let frequencies = statistics.frequencies();
        ...
        let distance_copy_count = u64::try_from(census.distance_symbols())
            .map_err(|_| EncodeError::output_size_overflow())?;
        if green_copy_count != distance_copy_count
            || census.copy_tokens() != census.distance_symbols()
        {
