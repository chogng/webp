...
fn wrap_vp8l(payload: Vec<u8>) -> Result<Vec<u8>, EncodeError> {
    webp_mux::serialize_vp8l(payload, 0, 0, false, webp_mux::Metadata::default())
...
fn wrap_vp8(
    ...
    let alpha_payload = alpha
        ...
        .transpose()?;
    webp_mux::serialize_vp8(payload, width, height, alpha_payload.as_deref())
        .map_err(map_container_error)
}
