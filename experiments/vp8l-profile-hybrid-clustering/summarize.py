...
RAW = ROOT / "raw" / "phase-a-102-final-binary" / "phase-a.tsv"
E37 = {"compact": 617_958_802, "low-latency": 625_321_072}
E40 = {"compact": 599_398_064, "low-latency": 617_047_520}
E41 = {"compact": 547_448_078, "low-latency": 601_400_998}
...
def summarize_profile(profile, rows, steps):
    ...
    exact = {
        "planner_writer_plan_rows": sum(
            3 + (integer(row, "split_riff_bytes") != 0) for row in selected
        ),
    ...
    return {
        ...
        "e37_bytes": E37[profile],
        "e40_bytes": E40[profile],
        "e41_bytes": E41[profile],
        "final_bytes": final,
        "vs_control_pct": pct(final, control),
        "vs_e37_pct": pct(final, E37[profile]),
        "vs_e40_pct": pct(final, E40[profile]),
        "vs_e41_pct": pct(final, E41[profile]),
...
def summarize_formal():
    path = ROOT / "raw/formal-102x5"
    if not path.exists():
        return None
    samples = process_samples(path)
    profiles = {}
    for profile, control, candidate, absolute_limit_ns in [
        ("compact", "compact-control", "compact", 7_100_000_000),
        ("low-latency", "low-latency-control", "low-latency", 6_900_000_000),
    ]:
        result = pair_summary(samples, control, candidate, expected_rounds=5)
        result["absolute_limit_ns"] = absolute_limit_ns
        result["gates"] = {
            "absolute_candidate_median":
                result["aggregate_candidate_ns"]["median"] <= absolute_limit_ns,
            "zero_per_image_median_regressions": result["median_time_regressions"] == 0,
        }
        result["formal_gate"] = all(result["gates"].values())
    ...
    result = {
        ...
        "profiles": profiles,
        "formal_gate": all(profile["formal_gate"] for profile in profiles.values()),
    }
...
def main():
    ...
    if screen_run.exists():
        ...
        binary_sha256 = "05b8421c86f3286667c5cffef35ffc2bff77f68d944063a954793e2b870e64c9"
        binary_bytes = 2_230_912
    summary = {
        "binary_sha256": binary_sha256,
        "binary_bytes": binary_bytes,
        "manifest_sha256": "9feb09f469753c43864011aa6f00cfc5ee1bd48da5aac8f9b16d105890e14f86",
        "profiles": {
            profile: summarize_profile(profile, rows["p18"], rows["p18-step"])
    ...
    exact = all(
        profile["exactness"][key]
        == (
            306
            if key == "planner_writer_plan_rows" and profile is compact
            else 408
            if key == "planner_writer_plan_rows"
            else 102
        )
        for profile in [compact, low]
        for key in profile["exactness"]
    )
    tails_pass = all(profile["images_over_2pct"] == 0 for profile in [compact, low])
    aggregate_pass = all(
        summary["profiles"][profile]["final_bytes"] <= E40[profile]
        for profile in E40
    )
    expected_totals = all(
        summary["profiles"][profile]["final_bytes"] == EXPECTED[profile]
        for profile in EXPECTED
    )
    ownership = compact["growth_attempts"] == 0 and compact["timings_ns"]["self_cost_ns"] == 0
    summary["gate"] = {
        "exactness_pass": exact,
        "expected_totals_reproduced": expected_totals,
        "aggregate_not_worse_than_e40": aggregate_pass,
        "both_profiles_within_e37_plus_2pct": tails_pass,
        "compact_constructed_no_growth_state": ownership,
        "phase_a_pass": exact and expected_totals and aggregate_pass and tails_pass and ownership,
    }
    ...
    formal = summarize_formal()
    if formal is not None:
        summary["formal"] = formal
    if screen is not None or formal is not None:
